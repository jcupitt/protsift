...
# 			puts "\tsequence = #{sequence}"
	...
	# find a list of all accession numbers of prots which contain the 
	# sequence
	...
	# find a list of all accession numbers of prots which match the
	# description
	def by_description(description)
		hits = []

		each do |protein|
			if protein.description.include?(description)
				hits << protein.accession
			end
		end
	...
	# run a block for every description, return a list of accession numbers
	# for which the block returned true
	def filter
		hits = []
	...
	def delete_if
		@db.delete_if do |accession, protein|
			yield protein
		end
	end
end
